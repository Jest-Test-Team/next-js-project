...
async function getWeatherData(location: string) {
  ...
  const url = `https://opendata.cwa.gov.tw/api/v1/rest/datastore/F-C0032-001?Authorization=${API_KEY}&locationName=${encodeURI(
    location
  )}`;
...
export default async function HomePage({
  ...
  const weatherInfo = {
    ...
    wx:
      weatherElements.find((el: WeatherElement) => el.elementName === "Wx")
        ?.time[0]?.parameter.parameterName || "-",
    pop:
      weatherElements.find((el: WeatherElement) => el.elementName === "PoP")
        ?.time[0]?.parameter.parameterName || "-",
    minT:
      weatherElements.find((el: WeatherElement) => el.elementName === "MinT")
        ?.time[0]?.parameter.parameterName || "-",
    maxT:
      weatherElements.find((el: WeatherElement) => el.elementName === "MaxT")
        ?.time[0]?.parameter.parameterName || "-",
    ci:
      weatherElements.find((el: WeatherElement) => el.elementName === "CI")
        ?.time[0]?.parameter.parameterName || "-",
  };
        ...
        {location && (
          <div className="space-y-4">
            <div className="flex justify-between items-center p-4 bg-white/20 rounded-lg">
              ...
              <span>
                {weatherInfo.minT}° / {weatherInfo.maxT}° C
              </span>
            </div>
            <div className="flex justify-between items-center p-4 bg-white/20 rounded-lg">
